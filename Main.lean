    ...
    | succ (succ x) => by have rs := rs; cases (mod3 x) with
      ...
      | threeOne j  => cases (parity j) with
        ...
        | odd k  => cases (mod3 k) with
          | threeZero l =>
              have sin := rs (succ (succ (2 * l - 2))) (m₁₂₁ l);
              have ext := singleToExts (2 * l - 2) sin; rw [m₀₃₂ l] at ext;
              exact SingleLimited.is12 l ext;
          | threeOne l  =>
              have sin := rs (succ (succ (4 * l + 1 - 2))) (m₁₃₁ l);
              have ext := singleToExts (4 * l + 1 - 2) sin; rw [m₁₃₂ l] at ext;
              exact SingleLimited.is13 l ext;
          | threeTwo l  =>
              have sin := rs (succ (succ (8 * l + 7 - 2))) (m₁₄₁ l);
              have ext := singleToExts (8 * l + 7 - 2) sin; rw [m₁₄₂ l] at ext;
      ...
      | threeTwo j  => cases (parity j) with
        ...
        | odd k  => cases (parity k) with
          ...
          | odd l  => cases (mod3 l) with
            | threeZero m =>
                have sin := rs (succ (succ (2 * m - 2))) (m₀₃₁ m);
                have ext := singleToExts (2 * m - 2) sin; rw [m₀₃₂ m] at ext;
