...
inductive ExtsLimited : Nat → Prop where
  | is : (l : Nat)
      ...
      → (ExtsLimited <| l+(l-1)/8+1)                -- 06' 割り算も計算すると Nat に落ち着くので問題無し
      → (ExtsLimited <| l+(l-2)/4*5+6)              -- 07'
      → (ExtsLimited <| 4*l+(l-1)/2+10)             -- 08'
      → (ExtsLimited <| 9*l+14)                     -- 04'
      → (ExtsLimited <| (9*l+25)/2)                 -- 05'
      → (ExtsLimited <| (9*l+11)/2)                 -- 13'
      → (ExtsLimited <| (9*l+1)/4)                  -- 14'
        → (ExtsLimited <| l)
...
inductive SingleLimited : Nat → Prop where
  ...
  | is05 : (m : Nat) → (ExtsLimited <| 8*m+7-2) → (SingleLimited <| succ (succ (succ (succ (succ (succ (succ (succ (m * 3)) * 2) * 2) * 3))))) -- !
